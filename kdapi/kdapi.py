...
proxies = {
    # "http": "http://xx.xx.xxx.xxx:xxxx",
}


@RateLimited(.2)  # Don't send more than two request per second
def check(url, getLessSimilar=False):
    is_reddit_link = parse(url if url.startswith("http://") else "http://" + url).hostname.split('.')[-2:][
                         0] == "reddit"
