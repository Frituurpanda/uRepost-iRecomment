...
# ### CONFIGURATION ### #
REFRESH_MARGIN = 60
# reddit redirect uri contains: http://127.0.0.1/
...
class OAuth2UtilRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.wfile.write(
            "Thank you for using OAuth2Util. The authorization was successful, you can now close this window.".encode(
                "utf-8"))


class OAuth2Util:
    def __init__(self, reddit, oauthappinfo_configfile="oauthappinfo.txt", oauthconfig_configfile="oauthconfig.txt",
                 oauthtoken_configfile="oauthtoken.txt"):
        self.r = reddit
        self.token = None
    ...
    def _read_app_info(self):
        try:
            lines = []
            with open(os.path.join(os.path.dirname(__file__), self.OAUTHAPPINFO_CONFIGFILE), "r") as f:
                for line in f:
                    if line.strip():
                        lines.append(line.strip())
            # extra slash after {9} to comply with http://127.0.0.1/'s extra slash
            self.r.set_oauth_app_info(lines[0], lines[1], "http://{0}/".format(REDIRECT_URL))
        except OSError:
            print(self.OAUTHAPPINFO_CONFIGFILE, "the OAUTHAPPINFO_CONFIGFILE was not found.")

    def _read_config(self):
        try:
            lines = []
            with open(os.path.join(os.path.dirname(__file__), self.OAUTHCONFIG_CONFIGFILE), "r") as f:
                for line in f:
                    if line.strip():
                        lines.append(line.strip())
            ...
            print(self.OAUTHCONFIG_CONFIGFILE, "OSError, OAUTHCONFIG_CONFIGFILE was not found.")

    def _read_access_credentials(self):
        try:
            lines = []
            with open(os.path.join(os.path.dirname(__file__), self.OAUTHTOKEN_CONFIGFILE), "r") as f:
                for line in f:
                    if line.strip():
                        lines.append(line.strip())
            self.token = lines[0]
            ...
            print("OSError: praw.errors.OAuthInvalidToken, Request new Token")
            self._get_new_access_information()
            print("After _get_new_access_information()")

    # ### SAVE SETTINGS ### #
    def _save_token(self):
        with open(os.path.join(os.path.dirname(__file__), self.OAUTHTOKEN_CONFIGFILE), "w") as f:
    ...
    def refresh(self):
        ...
        self.valid_until = time.time() + 2500
        self._save_token()
